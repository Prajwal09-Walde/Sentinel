...
app = FastAPI(title="TalentIQ Recruitment Matching Copilot")
...
def real_retrieval(query: str) -> List[str]:
    """Retrieve matching candidate profiles using skill and text keyword scoring."""
    if not CANDIDATE_POOL:
        return ["Resume: General software developer with experience in Git and Agile development."]
    
    query_words = set(query.lower().split())
    scored_candidates = []
    for cand in CANDIDATE_POOL:
        match_source = " ".join(cand.get("skills", [])) + " " + cand.get("experience", "") + " " + cand.get("education", "")
        source_words = set(match_source.lower().split())
    ...
    top_matches = []
    for score, cand in scored_candidates[:2]:
        if score > 0:
            profile = (
                f"Candidate: {cand.get('name')}\n"
                f"Skills: {', '.join(cand.get('skills', []))}\n"
                f"Experience: {cand.get('experience')}\n"
                f"Education: {cand.get('education')}"
            )
            top_matches.append(profile)
...
def real_generation(query: str, contexts: List[str]) -> str:
    """Generate professional recruitment match summary using OpenAI GPT model and contexts."""
    context_text = "\n\n---\n\n".join(contexts)
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are an expert HR Recruiter and Talent Advisor. "
                        "Evaluate and summarize the matching candidate profiles based strictly on the provided context "
                        "to answer the hiring manager's query. Detail why they match or mismatch. "
                        "Be professional, objective, and structured.\n\n"
                        f"Retrieved Candidate Contexts:\n{context_text}"
                    )
                },
        ...
        if "python" in query_lower:
            return "Matching Candidates:\n- Alice Smith (Senior Software Engineer, 5 yrs exp, MIT, Python/FastAPI/React/MongoDB/Docker).\nMatches criteria for backend development."
        elif "frontend" in query_lower:
            return "Matching Candidates:\n- Bob Johnson (Backend Engineer, 3 yrs exp, Stanford, JS/Node/Express/PostgreSQL/AWS).\nMatches criteria for web systems."
        elif "ai" in query_lower or "learning" in query_lower or "ml" in query_lower:
            return "Matching Candidates:\n- Diana Prince (Machine Learning Engineer, 4 yrs exp, Carnegie Mellon, PyTorch/TensorFlow/SQL).\nMatches criteria for ML/AI engineering."
        return f"Talent Pool Response: For query '{query}', please consult HR or candidate profiles: {context_text[:120]}..."
...
def run_query(request: QueryRequest) -> Dict[str, Any]:
    ...
    domain = "recruitment matching and resume screening"
    ...
    print(f"[LOG] Candidate Query evaluated. Passed: {eval_result.passed}, Route: {eval_result.generated_answer}")
...
def get_eval_metrics(n: int = Query(default=1000, ge=1), format: str = Query(default="html")) -> Any:
    ...
    <head>
        ...
        <title>TalentIQ Recruitment Matching Copilot Dashboard</title>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-800 pb-6 mb-8">
                <div>
                    <h1 class="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">
                        💼 TalentIQ Recruitment Matching Copilot
                    </h1>
            ...
            <div class="bg-slate-900/40 border border-slate-800 rounded-lg p-6 mb-8 hover:border-indigo-500/30 transition-all">
                ...
                <form id="sandbox-form" class="space-y-4">
                    <div class="flex flex-col">
                        ...
                        <textarea id="query-input" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-md p-3 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 transition-colors" placeholder="Type a recruitment matching question (e.g. Looking for a senior Python developer or Frontend developer with React experience)...."></textarea>
                    </div>
